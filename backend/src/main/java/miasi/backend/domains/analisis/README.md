...
Trzeba będzie dodać jeszcze jakiś przelicznik wagi gdzieś. Może to? A może enum? Nie jestem pewna, zostawiam w takim razie wam decyzję, żeby nie utrudnić przypadkowo

```java
package miasi.backend.config;

import miasi.backend.enums.ResourceType;
import miasi.backend.domains.analisis.types.core.ResourceWeightDictionary;
