...
public record NominalSimulationResponse(
    String status, String sessionId, SimulationVariant nominalVariant) {
  public static NominalSimulationResponse fromDomain(NominalSimulationSession session) {
    return new NominalSimulationResponse("success", session.getId(), session.getNominalVariant());
  }
}
