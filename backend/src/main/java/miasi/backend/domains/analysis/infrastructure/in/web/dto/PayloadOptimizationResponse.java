...
public record PayloadOptimizationResponse(
    String status, String sessionId, OptimalConfiguration configuration) {
  public static PayloadOptimizationResponse fromDomain(PayloadOptimizationSession session) {
    return new PayloadOptimizationResponse("success", session.getId(), session.getConfiguration());
  }
}
